            ...
            cleanSource: function() {
                ...
                html = html.replace(/<[^>]*>/g, function(match) {
                    ...
                    match = match.replace(/ ([^=]+)="?([^"]*)"?/g, function(match, attribute, value) {
                        if ($.inArray(attribute, settings.allowedAttributes) == -1)
                            return '';
                        switch (attribute) {
                            case 'id':
                                if ($.inArray(value, settings.allowedIDs) == -1)
                                    return '';
                            case 'class':
                                if ($.inArray(value, settings.allowedClasses) == -1)
                                    return '';
                            default:
                                return match;
                        }
...
*/
        ...
        $.extend(this, {
            ...
            checkState: function(uEditor, resubmit) {
                if (!resubmit) {
                    ...
                    setTimeout(function() {
                        uEditor.toolbar.checkState(uEditor, true);
                        return true;
                    }, 500);
                    return true;
                }
                ...
                while (!parentnode.is('body')) {
                    if (parentnode.is('a'))
                        uEditor.toolbar.setState("link", "on");
                    else if (parentnode.is('em'))
                        uEditor.toolbar.setState("italic", "on");
                    else if (parentnode.is('strong'))
                        uEditor.toolbar.setState("bold", "on");
                        ...
                        uEditor.toolbar.setState("formatblock", parentnode[0].nodeName.toLowerCase());
                    parentnode = parentnode.parent();
                }
            },
            ...
            setState: function(state, status) {
                if (state != "SelectBlock")
                    $(this.itemsList).find('.' + $.uEditorToolbarItems[state].className).addClass('on');
                else
                    $(this.itemsList).find('.' + $.uEditorToolbarItems[state].className).val(status);
            },
    ...
    var uEditorToolbarItems = function() {
        ...
        if (typeof (uEditorToolbarItemsClass.singleton) != 'undefined')
            return uEditorToolbarItemsClass.singleton;
        else
            uEditorToolbarItemsClass.singleton = this;
    ...
    $.fn.extend({
        ...
        appendToSelection: function(nodeType, attr, contentText, singleTag) {
            ...
            if ($.browser.msie) {
                var html;
                html = '<' + nodeType;
                $.each(attr, function(label, value) {
                    html += ' ' + label + '="' + value + '"'
                });
                if (singleTag)
                    html += ' />';
                    ...
                    html += '>';
                    if (contentText && typeof (contentText) != 'undefined')
                        html += contentText;
                    html += '</' + nodeType + '>';
                }
                selection = iframe.contentWindow.document.selection;
                range = selection.createRange();
                if ($(range.parentElement()).parents('body').is('#iframeBody '))
                    return;
                ...
                $(element).attr(attr);
                if (contentText && typeof (contentText) != 'undefined')
                    $(element).append(document.createTextNode(contentText));
